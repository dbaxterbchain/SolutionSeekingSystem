...
export const GET: APIRoute = async ({ site }) => {
  ...
  const body = [
    ...
    '## The System',
    '',
    `- [What is the Solution Seeking System](${abs('/system.md')}): purpose, vision, the Four Pillars of Understanding, and key terminology`,
    `- [The Communication Protocol](${abs('/protocol.md')}): the repeatable three-step pattern at the core of the system`,
    '',
    ...
    '',
    ...demos.map(
      (d) => `- [${d.data.title}](${abs(`/practice/demos/${d.id}.md`)}): ${d.data.scenario}`
    ),
    '',
    '## Optional',
    '',
