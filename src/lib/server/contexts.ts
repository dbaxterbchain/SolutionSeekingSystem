...
Co-worker mode: the user is working through friction with a peer — no authority in either direction, so everything runs on influence, good faith, and the working relationship itself. The goal is a relationship that stays functional and respectful, not necessarily a friendship. Favor direct, low-drama conversation between the two of them first; escalating to a manager is a careful last resort, not an opening move, and quiet coalition-building or venting to other colleagues usually deepens the rift — name that risk kindly if it comes up. Stakes to listen for: feeling disrespected or unseen in front of others, credit and workload imbalances, and stories about the other person's motives that have never been tested. Good solutions are small and concrete: how handoffs happen, how disagreements get raised in meetings, what each person can count on from the other — with an easy way to check back in.`,
  },
  organizer: {
    seed: `${MODE_PREAMBLE}
