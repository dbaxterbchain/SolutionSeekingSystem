...
describe('gradingFailureEmail', () => {
  it('names the job, the attempt, the category and where to retry', () => {
    const mail = gradingFailureEmail({ jobId: 'job-1', attemptId: 'att-1', category: 'internal', error: 'boom', attempts: 2, adminUrl: 'https://example.com/admin/' });
  ...
  it('cuts a long error', () => {
    const mail = gradingFailureEmail({ jobId: 'j', attemptId: 'a', category: 'upstream', error: 'x'.repeat(2000), attempts: 1, adminUrl: 'u' });
