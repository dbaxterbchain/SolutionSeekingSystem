...
- [ ] **5. Production.** Stays `hidden`. When David is satisfied, the branch is merged through PR #16, opened after PR #15 merged on 2026-09-10 with sub-plans 1a and 1b only (the finishing skill's menu: merge locally, PR, or keep).
...
Task 8 (ship) on 2026-09-11, after David merged PR #16 and gave the go-ahead. The hosted database already had `0030` and `0031` applied at their current content when the controller looked: twelve tables with RLS on, no policies and service-role-only grants; the eight functions with the 1e signatures (`claim_course_grading_job` defaulting to 840 s); the security advisors reporting the twelve course rows and nothing else new; every course table answering 401 to the publishable key; the three sequences granted to the service role only. Netlify: `COURSE_WORKER_SECRET` set as a secret in production, deploy-preview and branch-deploy; `COURSE_AWARDS_ENABLED=false`, `COURSE_GRADER_MODEL=claude-opus-5` and `CLOUDFLARE_STREAM_CUSTOMER_CODE` in every context; `PUBLIC_COURSE_STATUS=hidden` in production and `preview` on the `course-beta` branch context, which also carries `COURSE_SAMPLE_FORMS=true`, the test-mode `STRIPE_PRICE_ID_COURSE` and the signing secret of a new test-mode webhook endpoint at the branch's address with the six events. Ruling: the branch context runs `preview`, never `open`, because an `open` build refuses placeholder videos and unset launch tokens by design, and an admin can still check out in `preview`. David added the two async payment events to the live endpoint himself. The stand-in clip reached Stream by URL from a temporary unpublished draft deploy, deleted afterwards: UID `07811cf467f0257738287c6a95a5ec10`, signed URLs on, English captions ready, and `COURSE.placeholderStreamUid` names it on branch `course-beta` (PR #17). The worker refused a POST without the secret on the PR #17 preview and on production (202 to the caller, `refused (missing or wrong worker secret)` in the function log), and the production deploy registered the sweeper's schedule. Netlify applies a branch context's values to deploy previews built from that branch, so the PR #17 preview already runs in preview mode. Left for David because the session's permission classifier blocked them: allowing `course-beta` as a branch deploy, adding the branch hostname to the Turnstile widget, the test-mode `STRIPE_SECRET_KEY` on the branch context, and the two test accounts the controller wanted for a hosted grade; and, by design, the Stream API token, the grant and revoke from `/admin`, and the assessment run on the deploy.
